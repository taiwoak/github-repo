...
const Repo = () => {
    ...
    useEffect(() => {
        fetch(`https://api.github.com/repos/taiwoak/${id}`)
      ...
      useEffect(() => {
        fetch(`https://api.github.com/repos/taiwoak/${id}/deployments`)
        .then((response) => (response.json()))
        .then((data) => {
          setDeployment(data)
        })
      ...
      return (
        <div id="singlerepo" className='d-flex flex-row align-items-center justify-content-center'>
            <div className="singlerepocard">
                ...
                <p>Live site: {deployment.length === 0 ? `none` : <a href={`https://taiwoak.github.io/${details.name}`}>taiwoak.github.io/{details.name}</a>}</p>
